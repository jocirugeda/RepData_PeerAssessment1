...
```
- ``r sprintf("%.2f ", daily_average)  `` Is The mean total number of steps per day
- ``r sprintf("%d ", daily_median) `` Is The median for the total number of steps per day 
...
plot(ds_by_interval$interval,ds_by_interval$avg_time_steps,type = "l",xlab="Time interval",ylab="Average steps")
...
The interval that the maximum number of steps is the  ``r sprintf("%d ", max_time) `` 
...
The number of row with NA data is ``r sprintf("%d",size_na) ``


2- Devise a strategy for filling in all of the missing values in the dataset. The strategy does not need to be sophisticated. For example, you could use the mean/median for that day, or the mean for that 5-minute interval, etc.

```{r}
# using the interval mean of steps , we can fill the NA values  with the mean for that interval


# merge by column interval to generate a dataset with each interval mean asigned 
...
- ``r sprintf("%.2f ", daily_average)  `` Is The mean total number of steps per day
- ``r sprintf("%.2f ", daily_median) `` Is The median for the total number of steps per day 
...
# split of Data set 
ds_weekdays<-sort_new_ds[sort_new_ds$DayNumber!=0 & sort_new_ds$DayNumber!=6,]

ds_weekends<-sort_new_ds[sort_new_ds$DayNumber==0 | sort_new_ds$DayNumber==6,]
...
plot(weekdays_by_interval$interval,weekdays_by_interval$avg_time_steps,type = "l",xlab="Time interval",ylab="Average steps",main=" Weekdays")


plot(weekends_by_interval$interval,weekends_by_interval$avg_time_steps,type = "l",xlab="Time interval",ylab="Average steps",main=" Weekends")
